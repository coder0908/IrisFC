...
bool rp3_init()
{
	...
	status = HAL_UART_Receive_DMA(s_rp3_huart, s_uart_rx_buf, UART_LEN_RXBUF);
	if (status != HAL_OK) {
		return false;
	}
	return true;
...
void rp3_flush_tx()
{
	...
	while (true) {
		ret = crsf_pop_frame_queue(&s_crsf_tx_queue, &frame);
		if (!ret) {
			break;
		}
...
void rp3_flush_rx()
{
	...
	if (HAL_UART_Receive_DMA(s_rp3_huart, s_uart_rx_buf, UART_LEN_RXBUF) != HAL_OK) {
		assert(false);
		return;
	}
